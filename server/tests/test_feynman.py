...
def _mock_feynman_response(mocker):
...
class TestFeynmanAPI:
    ...
    def test_evaluate_returns_200_with_mocked_llm(self, client: TestClient, mocker):
        """正常请求返回 200，字段映射正确"""
        _mock_feynman_response(mocker)
        ...
        assert dev["userSegment"] == "用户表述中的一部分"  # camelCase
        ...
        assert gl["toSegmentId"] == "seg-001"  # camelCase

    def test_evaluate_empty_segments(self, client: TestClient, mocker):
    ...
    def test_evaluate_missing_user_id(self, client: TestClient, mocker):
        """缺少 X-User-Id 返回 400"""
        _mock_feynman_response(mocker)
